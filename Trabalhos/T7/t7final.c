...
// verifica a quantidade de um dado registro nos arquivos
...
// projetada para calcular o tamanho (quantidade de dígitos) de um número inteiro
int tamNum(int num)
{
    int tam = 0;
    while (num != 0)
...
// projetada para verificar se uma string contém apenas dígitos
int verif(char ent[])
{
    int tam = strlen(ent);
    int conf = 0;
...
int verFabMod(char val[], int tipo)
{
    ...
    if (tipo == 0)
    {
        char fabricantes[4][100] = {"Audi", "Honda", "Kia", "Lamborghini"};
        int cont;
        for (int i = 0; i < 4; i++)
        {
            cont = 0;
            if (strlen(val) == strlen(fabricantes[i]))
            {
                for (int j = 0; j < tam; j++)
                {
                    if (val[j] == fabricantes[i][j])
                        cont++;
                }
                if (cont == tam)
                {
    ...
    else if (tipo == 1)
    {
        ...
        {
            cont = 0;
            if (strlen(val) == strlen(modelos[i]))
            {
                for (int j = 0; j < tam; j++)
                {
                    if (val[j] == modelos[i][j])
                        cont++;
                }
                if (cont == tam)
                {
...
// versão de verificaçao de quant de resgistros com float
...
// printa os dados de um cliente especifico
void infDadosCliente(struct CLIENTE CLIENTE)
{
    ...
    printf("    Cep: ");
    for (int i = 0; i < 10; i++)
        printf("%c", CLIENTE.endereco.cep[i]); // Imprime o CEP do endereço
    printf("\nTelefone Residencial: %s\n", CLIENTE.residencial.telefone);
    // Imprime os números de celular do cliente
    printf("Celular: ");
    for (int i = 0; i < 5; i++)
    {
        // Verifica se o próximo número de celular é válido (caso o próximo comece com '0' ou seja o último)
        if (CLIENTE.celular[i + 1].telefone[0] == '0' || i == 4)
        {
            printf("\"%s\"\n", CLIENTE.celular[i].telefone);
            i = 5; // Sai do loop, pois encontrou o último número ou um número inválido
        }
        else
        {
            printf("\"%s\", ", CLIENTE.celular[i].telefone); // Imprime o número de celular seguido de uma vírgula
        }
    }
    printf("Renda mensal: %.2f\n", CLIENTE.renda_mensal); // imprime renda mensal
    printf("------------------------------------------------------------------------------\n");
}

// compara CPF verifica se é igual ao fornecido
...
void gerar_tele_movel(char *ptr_telefone, int modificador)
{
    ...
    for (int i = 2; i < 14; i++)
    {
        do
        {
            telefone_int[i] = rand() % 9 + modificador; // modificador serve para randomizar melhor, os numeros de telefone do cliente
            if (telefone_int[i] > 9)
            {
                telefone_int[i] -= 2; // evita que o valor passe de 9
            }

        } while (telefone_int[i] > 9); // impede que telefone_int[i] > 9
    }

    telefone_int[2] = 9; // adiciona como primeiro digito o 9, por padrão
    ...
    ptr_telefone[13] = '\0'; // finalixa a frase
    ptr_telefone[2] = 32;    // cria um espaço entre o DDD e o numero de telefone
}

// gera um nome valido com Nome_Principal e Nome_sobrenome
void gerar_nome_valido(char *ptr_nome)
{
    ...
    // copia o nome e espeaço para a string
    strcpy(ptr_nome, nomes[a]);
    strcat(ptr_nome, " ");
    strcat(ptr_nome, sobrenomes[b]);
    strcat(ptr_nome, "\0"); // finaliza a frase
}

// cria um CPF valido baseado no padrão do mesmo.
void gerar_cep_valido(char *ptr_cep)
{
    srand(time(NULL));
    int cep_int[10]; // versão int para randomizar parte dos nuemros
    ...
    {
        if (i == 2)
            ptr_cep[2] = 46; // ponto na tabela asc
        else if (i == 6)
            ptr_cep[6] = 45; // traço na tabela asc
        else
...
{
    // gera um grupo de endereço completo
    ...
    *ptr_numero_casa = rand() % 727;
    ...
    strcat(ptr_cep, "\0"); // finaliza o CEP
}
...
void gerar_telefone_fixo(char *ptr_tele_fixo)
{
    ...
    for (int i = 0; i < 14; i++)
    {
        ...
        {
            ptr_tele_fixo[i] = conversor_char(telefone_int[i]); // converte para char os valores
        }
    }
    ptr_tele_fixo[2] = 32;    // cria espaço entre o DDD e o numero
    ptr_tele_fixo[13] = '\0'; // finaliza a frase
}
...
{
    srand(time(NULL));
    char val[2];   // Variável para armazenar a resposta do usuário
    int conf = -1; // Variável de confirmação inicializada como inválida
    limpa();
    // Solicita a confirmação do usuário para prosseguir com a venda
    printf("Cliente e carro localizados, deseja prosseguir com a venda?\n1 - Sim\n0 - Nao\n");
    scanf("%s", val);
    // Verifica se a resposta é válida
    if (verif(val) == 1)
    {
        conf = atoi(val); // Converte a resposta para um número inteiro

        if (conf == 1)
        {
            struct VENDA_CARRO venda;
            // faz as copias das informações
            ...
            if (venda.data_venda.mes != 2)
                venda.data_venda.dia = (rand() % 28) + 1; // Define um dia aleatório para a data da venda
            else
                venda.data_venda.dia = (rand() % 30) + 1;
            venda.data_venda.ano = (rand() % 43) + 1980; // Gera um ano entre 1980 e 2022

            struct SUB_DADOS_VENDA subDados;
            // Cria uma estrutura para dados adicionais da venda
            // Copia informações relevantes sobre o carro e o cliente para a estrutura de venda
            strcpy(subDados.placa, carros[posCarro].placa);
            strcpy(subDados.nome, clientes[posCliente].nome);
            strcpy(subDados.modelo, carros[posCarro].modelo);
            strcpy(subDados.fabricante, carros[posCarro].fabricante);
            subDados.anoFab = carros[posCarro].ano_fabricacao;

            FILE *arqVenda;
            arqVenda = fopen(diretorios[2], "ab"); // Abre o arquivo de vendas para adicionar os dados
            fwrite(&venda, sizeof(venda), 1, arqVenda);
            fclose(arqVenda);

            FILE *arqSubVenda;
            arqSubVenda = fopen(diretorios[3], "ab"); // Abre o arquivo de dados adicionais da venda
            fwrite(&subDados, sizeof(subDados), 1, arqSubVenda);
            fclose(arqSubVenda);

            FILE *arqCarro;
            arqCarro = fopen(diretorios[0], "wb"); // Abre o arquivo de carros para escrita
            ...
            return 1; // Retorna 1 indicando sucesso na venda
        }
        else if (conf == 2)
        {
            limpa();
            printf("Venda cancelada\n\n");
            return 0; // Retorna 0 indicando venda cancelada
        }
        else
        {
...
// que normaliza um texto convertendo a primeira letra para maiúscula e as letras subsequentes para minúsculas
void norm(char text[])
{
    int quant = strlen(text); // Calcula o tamanho da string passada como argumento
    char str[quant];          // Cria um array para armazenar a cópia do texto
    strcpy(str, text);        // Faz uma cópia do texto original para não alterar diretamente
    for (int i = 0; i < quant; i++)
    {
        if (i == 0)
            text[i] = toupper(str[i]); // Converte o primeiro caractere para maiúscula
        else
            text[i] = tolower(str[i]); // Converte os caracteres restantes para minúscula
    }
}

// printa informações do carro
void listarFabr(struct VENDA_CARRO venda, struct SUB_DADOS_VENDA subVend)
{
    printf("Modelo: %s\n", subVend.modelo);
    printf("Placa: %s\n", venda.placa_car);
    printf("Ano de Fabricacao: %d\n", subVend.anoFab);
...
// imprimir informações específicas de uma venda de carro
void listarMod(struct VENDA_CARRO venda, struct SUB_DADOS_VENDA subVend)
{
    // Imprime informações específicas da venda e seus dados adicionais
    printf("Ano de Fabricacao: %d\n", subVend.anoFab);
    printf("Placa: %s\n", venda.placa_car);
    printf("Nome do cliente: %s\n", subVend.nome);
    printf("------------------------------------------------------------------------------\n");
...
// imprimir informações detalhadas sobre uma venda de carro e seus dados associados
void infDadosVenda(struct VENDA_CARRO venda, struct SUB_DADOS_VENDA subVenda)
{
    // Imprime informações da estrutura 'struct VENDA_CARRO'
    printf("Placa: %s\n", venda.placa_car);
    printf("Cpf: %s\n", venda.cpf_cli);
    printf("Preco: %.2f\n", venda.preco_venda);
    printf("Data: %d/%d/%d\n\n", venda.data_venda.dia, venda.data_venda.mes, venda.data_venda.ano);

    //  Imprime informações da estrutura 'struct SUB_DADOS_VENDA'
    printf("Placa: %s\n", subVenda.placa);
    printf("Nome: %s\n", subVenda.nome);
    printf("Modelo: %s\n", subVenda.modelo);
    printf("Fabricante: %s\n", subVenda.fabricante);
...
// a ordenar clientes por nome
void ordenaClientesPorNome(char diretorio[TAM])
{
    int quant = quantRegistro(diretorio, 2); // Obtém a quantidade de registros no arquivo

    if (quant > 0)
    {
        struct CLIENTE cliente_ordenado[quant]; // Cria um array para armazenar os clientes ordenados

        FILE *arq;
        arq = fopen(diretorio, "rb");

        fread(cliente_ordenado, sizeof(struct CLIENTE), quant, arq); // Lê os registros do arquivo para o array
        fclose(arq);
        // Algoritmo de ordenação (Bubble Sort) para ordenar os clientes por nome
        for (int i = 0; i < quant - 1; i++)
        {
            for (int j = 0; j < quant - i - 1; j++)
            {
        ...
        // Exibe as informações dos clientes ordenados
        for (int i = 0; i < quant; i++)
        {
...
void menuVenda(char diretorios[4][20])
{
    ...
    while (escolha != 0)
    {
        ...
        if (verif(ent) == 1)
        {
            ...
            { // Excluir uma venda
                int quant = quantRegistro(diretorios[2], 3);
                if (quant > 0)
                {
                    char placa[9];
                    limpa();
                    ...
                    struct VENDA_CARRO vendas[quant];
                    for (int i = 0; i < quant; i++)
                    {
                        fread(&vendas[i], sizeof(vendas[i]), 1, arqVenda);
                        if (strcmp(placa, vendas[i].placa_car) == 0)
                            pos = i;
                    }
                    fclose(arqVenda);
                    ...
                    {
                        FILE *arqSubVenda;
                        arqSubVenda = fopen(diretorios[3], "rb");
                        struct SUB_DADOS_VENDA subVendas[quant];
                        for (int i = 0; i < quant; i++)
                        {
                            fread(&subVendas[i], sizeof(subVendas[i]), 1, arqSubVenda);
                        }
                        fclose(arqSubVenda);

                        arqVenda = fopen(diretorios[2], "wb");
                        for (int i = 0; i < quant; i++)
                            if (i != pos)
                                fwrite(&vendas[i], sizeof(vendas[i]), 1, arqVenda);
                        fclose(arqVenda);
                        arqSubVenda = fopen(diretorios[3], "wb");
                        for (int i = 0; i < quant; i++)
                            if (i != pos)
                                fwrite(&subVendas[i], sizeof(subVendas[i]), 1, arqSubVenda);
                        fclose(arqSubVenda);
                        limpa();
                        printf("Historico de venda deletada com sucesso.\n\n");
                    }
                    else
                    {
                        err("Nao ha nenhuma venda com a placa do carro inserido!");
                    }
                }
                else
                {
                    err("Nao ha nenhuma venda cadastrada no sistema!");
                }
            }
            else if (escolha == 3)
            { // Listar os carros de um fabricante, ordenados pelo modelo
                ...
                if (quant > 0)
                {
                    char fab[TAM];
                    limpa();
                    printf("Digite o nome do fabricante para ver suas vendas: \n");
                    scanf("%s", fab);
                    norm(fab);
                    if (verFabMod(fab, 0) >= 0)
                    {
                        struct VENDA_CARRO vendas[quant];
                        struct SUB_DADOS_VENDA subVendas[quant];

                        FILE *arqVendas, *arqSubVendas;
                        arqVendas = fopen(diretorios[2], "rb");
                        arqSubVendas = fopen(diretorios[3], "rb");
                        ...
                        for (int i = 0; i < quant; i++)
                        {
                            fread(&vendas[i], sizeof(vendas[i]), 1, arqVendas);
                            fread(&subVendas[i], sizeof(subVendas[i]), 1, arqSubVendas);
                            if (verFabMod(fab, 0) == verFabMod(subVendas[i].fabricante, 0))
                            {
                                valores[cont][0] = i;
                                valores[cont][1] = verFabMod(subVendas[i].modelo, 1);
                                cont++;
                            }
                        }
                        fclose(arqVendas);
                        ...
                        {
                            limpa();
                            ordValores(cont, valores);
                            printf("------------------------------------------------------------------------------\n");
                            for (int i = 0; i < cont; i++)
                            {
                                listarFabr(vendas[valores[i][0]], subVendas[valores[i][0]]);
                            }
                            printf("\n");
                        }
                        else
                        {
                            limpa();
                            printf("Nao ha nenhuma venda para este fabricante no momento\n\n");
                        }
                    ...
                    {
                        err("Fabricante nao esta presente no sistema!");
                    }
                }
                else
                {
            ...
            { // Listar os carros de um modelo, ordenados pelo ano de fabricacao
                ...
                if (quant > 0)
                {
                    char mod[TAM];
                    limpa();
                    printf("Digite o modelo do carro para ver suas vendas:\n");
                    scanf("%s", mod);
                    norm(mod);
                    if (verFabMod(mod, 1) >= 0)
                    {
                        struct VENDA_CARRO vendas[quant];
                        struct SUB_DADOS_VENDA subVendas[quant];
                        ...
                        for (int i = 0; i < quant; i++)
                        {
                            fread(&vendas[i], sizeof(vendas[i]), 1, arqVendas);
                            fread(&subVendas[i], sizeof(subVendas[i]), 1, arqSubVendas);
                            if (verFabMod(mod, 1) == verFabMod(subVendas[i].modelo, 1))
                            {
                                valores[cont][0] = i;
                                valores[cont][1] = subVendas[i].anoFab;
                                cont++;
                            }
                        }
                        fclose(arqVendas);
                        ...
                        if (cont > 0)
                        {
                            ...
                            for (int i = 0; i < cont; i++)
                            {
                                listarMod(vendas[valores[i][0]], subVendas[valores[i][0]]);
                            }
                            printf("\n");
                        }
                        else
            ...
            { // Informar a quantidade de carros vendidos e seu total
                ...
                if (quant > 0)
                {
                    float total = 0.0;
                    FILE *arq;
                    arq = fopen(diretorios[2], "rb");
                    struct VENDA_CARRO vendas;
                    for (int i = 0; i < quant; i++)
                    {
                        fread(&vendas, sizeof(vendas), 1, arq);
                        total += vendas.preco_venda;
                    }
                    fclose(arq);
                    limpa();
                    printf("A quantidade de carros vendidos foi: %d\n\n", quant);
                    printf("E seus valores totalizados foi: R$: %.2f\n\n", total);
                }
                else
                {
                    err("Nao ha nenhuma venda cadastrada no sistema!");
            ...
            { // Informar o lucro total das vendas
                ...
                if (quant > 0)
                {
                    ...
                    fclose(arq);
                    limpa();
                    printf("O lucro total das vendas foi: R$: %.2f\n\n", total);
                }
                else
                {
                    err("Nao ha nenhuma venda cadastrada no sistema!");
